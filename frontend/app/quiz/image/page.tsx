...
import ImageQuizForm from "@/components/ImageQuizForm";
import ImageScoreResult from "@/components/ImageScoreResult";
...
function storageKey(userId: string) {
  return `quizdr:image-session:v1:${userId}`;
}
...
export default function ImageQuizPage() {
  ...
  async function startQuiz() {
    setStage("loading");
    try {
      const qs = await fetchImageQuestions(count);
      setQuestions(qs);
      setAnswers({});
      setCurrentIndex(0);
      setResult(null);
  ...
  async function handleSubmitQuiz() {
    ...
    try {
      ...
      const res = await submitImageQuiz(payload);
      setResult(res);
      setStage("result");
    } catch (e) {
      setError(String(e));
  ...
  return (
    <main className="mx-auto flex w-full max-w-xl flex-col gap-6 p-8">
      ...
      {stage === "result" && result && (
        <ImageScoreResult result={result} questions={questions} onRetake={resetToSetup} />
      )}
    </main>
  );
}
