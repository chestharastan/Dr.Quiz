...
def rasterize_pdf(
    ...
    out_dir = out_dir or Path(tempfile.mkdtemp(prefix="qcm_raster_"))
    out_dir.mkdir(parents=True, exist_ok=True)
    prefix = out_dir / "page"
    # pdftoppm prints nothing while it runs, so a big page range otherwise
    # looks like the whole process is frozen before any per-page output.
    print(
        f"  rasterizing pages {start_page}-{end_page} at {dpi} dpi...",
